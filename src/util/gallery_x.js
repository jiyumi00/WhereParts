        ...
        return(
            <SafeAreaView style={styles.container}>
                ...
                {this.state.group==true && (
                    <FlatList data={this.state.groupNames} horizontal={false} numColumns={3} renderItem={(item)=><GroupListItem item={item} itemSize={this.groupURIs[item.item].length} onSelectListener={(name)=>this.onSelectGroupListener(name)}/>}/>              
                )}
                {/* 전체 이미지를 볼 경우 */}
    ...
    render() {
        return(
            <TouchableOpacity onPress={()=>this.onSelectGroup(this.props.item.item)}>
                <View style={{paddingTop:'3%',paddingLeft:'2%',  /*borderWidth:1*/}}>
                    <Image source={ folder } style={styles.image} />
                    <Text style={{textAlign:"center"}}>{this.props.item.item} ({this.props.itemSize})</Text>
                </View>               
            </TouchableOpacity>
        );
    }
